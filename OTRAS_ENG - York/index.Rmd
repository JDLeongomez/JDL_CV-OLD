...
**About me:** I currently am an Associate Professor and Researcher at the [Human Behaviour Lab (LACH)](https://www.psicologia.unbosque.edu.co/lach), [Faculty of Psychology](https://www.uelbosque.edu.co/psicologia), at [Universidad El Bosque](https://www.uelbosque.edu.co/) in Bogota, Colombia. I hold a Bachelor’s
in Music Education from Universidad Pedagógica Nacional (Colombia), an MSc in Evolutionary Psychology
from the University of Liverpool, and a PhD from the University of Stirling.
...
#ggplot(pubs, aes(x = sqrt(square), y = sqrt(sums))) +
#  geom_segment(aes(x = gindex^2, y = 0, xend = gindex^2, yend = gindex^2),
#               size = 0.1, color = "#008080") +
#  geom_line(size = 1, colour = "grey") +
#  geom_abline(intercept = 0, slope = 1, colour = "#008080", linetype = "dashed")
```

I have published (or have in press) `r nrow(pubs)-4` journal articles. In most of these I have done all statistical analyses, and 12 are as first or last author. To date, my articles have been cited `r citSum` times, `r citRecentSum` (`r label_percent()(citRecentProp[1,1])`) since the beginning of `r yearRecent` (see figure below), and `r numbers_to_words(count50cit)` have been cited over 50 times. I have an h-index of `r profile$h_index` and a g-index of `r gindex`.

```{r echo=FALSE, fig.height=3, fig.width=7.3}
annotate_figure(p.fin, bottom = text_grob(paste0("Source: Google Scholar. Updated ", 
                                       format(Sys.Date(),'%B %d, %Y')),
                hjust = 1, x = 1, size = 6))
```

Between 2015 and 2017 my contracts were teaching centred, with little time for research. Since 2018, however, I have focused increasingly more on research.
